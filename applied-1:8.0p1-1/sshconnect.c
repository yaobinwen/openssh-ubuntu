...
check_host_cert(const char *host, const struct sshkey *key)
{
                                       ...
                                       options.ca_sign_algorithms)) != 0)
    {
        logit("%s: certificate signature algorithm %s: %s", __func__,
              (key->cert == NULL || key->cert->signature_type == NULL) ? "(null)" : key->cert->signature_type, ssh_err(r));
        return 0;
    }
...
sockaddr_is_local(struct sockaddr *hostaddr)
{
    ...
    {
    case AF_INET:
        return (ntohl(((struct sockaddr_in *)hostaddr)->sin_addr.s_addr) >> 24) == IN_LOOPBACKNET;
    case AF_INET6:
...
retry:
    ...
    case HOST_OK:
        ...
        if (want_cert &&
            !check_host_cert(options.host_key_alias == NULL ? hostname : options.host_key_alias, host_key))
            goto fail;
...
int verify_host_key(char *host, struct sockaddr *hostaddr, struct sshkey *host_key)
{
    u_int i;
    int r = -1, flags = 0;
